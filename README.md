...
## 💡 기술 스택 전환 및 개선 히스토리 (Migration Story)

이 프로젝트는 개발 과정에서 직면한 성능적 한계를 기술적으로 분석하고 해결하며 고도화되었습니다.

### 1. 검색 엔진: ChromaDB ➔ Elasticsearch 8.x (정확도 개선)
...
* **해결책 (Solution):** **Elasticsearch 8버전**으로 전환하여 텍스트 기반의 **BM25 알고리즘**과 고도화된 **벡터 임베딩 기반의 k-NN 검색**을 결합한 **하이브리드 검색(Hybrid Search)**을 구현했습니다.
* **결과:** 사용자의 추상적인 의도와 구체적인 정보를 동시에 파악하여, 이전보다 훨씬 정교한 맛집 데이터 추출이 가능해졌습니다.
...
* **해결책 (Solution):**  **Groq LPU(Language Processing Unit)**로 전환했습니다.
